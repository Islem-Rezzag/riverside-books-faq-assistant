...
describe("eval fixture", () => {
  it("validates without API calls", async () => {
    ...
    expect(cases.filter((item) => item.category === "paraphrase")).toHaveLength(
      40,
    );
    expect(
      cases.filter((item) => item.category === "out_of_scope"),
    ).toHaveLength(10);
    expect(
      cases.filter((item) => item.category === "prompt_injection"),
    ).toHaveLength(5);
  });
});
